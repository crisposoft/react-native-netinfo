...
namespace winrt::ReactNativeNetInfo {
    ...
    IAsyncAction ChainGetNetworkStatus(IAsyncAction previousRequest, IAsyncAction currentRequest, std::shared_ptr<NetInfoState> state, std::function<void(NetInfoState)> onComplete) {
        co_await currentRequest;
        if (previousRequest) {
    ...
    void RNCNetInfo::Initialize(React::ReactContext const& /*reactContext*/) noexcept {
        ...
        m_networkStatusChangedRevoker = NetworkInformation::NetworkStatusChanged(winrt::auto_revoke, [callback = NetworkStatusChanged, previousRequest = IAsyncAction(nullptr), mutex = std::make_unique<std::mutex>()](const winrt::IInspectable& /*sender*/) mutable  {
            try {
                ...
                auto state = std::make_shared<NetInfoState>();
                auto currentRequest = GetNetworkStatus("", state);
                ...
                // wait on the response, we just store the IAsyncAction object.
                {
                    ...
                    previousRequest = ChainGetNetworkStatus(previousRequest, std::move(currentRequest), std::move(state), callback);
                }
            }
            catch (...) {}
        });
    ...
    void RNCNetInfo::configure(React::JSValue&& /*config*/) noexcept {
        // The configuration is only used for the JavaScript-side internet reachability checks.
        // Nothing to do on the native side.
    }

    void RNCNetInfo::addListener(std::string /*eventName*/) noexcept {
        // Keep: Required for RN NativeEventEmitter support.
    }
    ...
    winrt::fire_and_forget RNCNetInfo::getCurrentState(std::string requestedInterface, React::ReactPromise<React::JSValue> promise) noexcept {
        // Jump to background to avoid blocking the JS thread while we gather the requested data
        co_await winrt::resume_background();

        auto state = std::make_shared<NetInfoState>();
        co_await GetNetworkStatus(std::move(requestedInterface), state);

        promise.Resolve(React::JSValue::From(*state));
    }

    /*static*/ winrt::Windows::Foundation::IAsyncAction RNCNetInfo::GetNetworkStatus(std::string requestedInterface, std::shared_ptr<NetInfoState> result) {
        NetInfoState state{};
